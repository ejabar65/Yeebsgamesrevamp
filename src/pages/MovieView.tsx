...
export default function MovieView({ typeOverride }: { typeOverride?: 'movie' | 'tv' }) {
  ...
  if (loading) {
    return (
      <div className="flex flex-col items-center justify-center min-h-screen bg-[#020202] gap-12">
        ...
        <p className="text-[12px] font-black uppercase tracking-[0.5em] text-gray-800 animate-pulse">Establishing Connection...</p>
      </div>
    );
  }
  ...
  if (!media) {
    return (
      <div className="flex flex-col items-center justify-center min-h-screen bg-[#020202] text-center p-6">
        <Activity className="w-12 h-12 text-red-500/20 mb-8" />
        <h1 className="text-4xl font-black text-white uppercase italic tracking-tighter mb-4">Signal Lost</h1>
        <p className="text-[10px] uppercase font-black tracking-widest text-gray-700 mb-12">Data coordinates unreachable or expired.</p>
        <button 
    ...
    <main className="min-h-screen bg-[#020202] text-white font-sans selection:bg-blue-500 pb-32">
      ...
      <header className="fixed top-0 inset-x-0 h-24 flex items-center justify-between px-10 z-50 backdrop-blur-md border-b border-white/5 bg-black/20">
        ...
        >
          ...
          <span className="text-[10px] font-black uppercase tracking-[0.3em] text-gray-500 group-hover:text-white transition-colors">Exit Protocol</span>
        </button>
        ...
        >
          ...
          <span className="hidden sm:inline">Offshore Link</span>
        </button>
      </header>
           ...
           >
             <div className="space-y-4">
                <div className="flex items-center gap-4">
                   ...
                   <p className="text-[11px] font-black text-blue-500 uppercase tracking-[0.6em]">{media.isCustom ? 'LOCAL ASSET' : type === 'movie' ? 'CINEMA' : 'SERIES'}</p>
                </div>
          ...
          >
             ...
             <div className="absolute top-6 left-6 opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none">
                <div className="flex items-center gap-3 px-4 py-2 bg-black/40 backdrop-blur-xl rounded-full border border-white/10">
                   ...
                   <span className="text-[9px] font-black uppercase tracking-widest">Active Stream: {activeSource.name}</span>
                </div>
             </div>
          </motion.div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-16 pt-12">
             <div className="lg:col-span-8 space-y-12">
                <div className="space-y-6">
                   <div className="flex items-center gap-4">
                      ...
                      <h2 className="text-[12px] font-black uppercase tracking-[0.5em] text-gray-700">Archival Abstract</h2>
                   </div>
             ...
             <div className="lg:col-span-4 space-y-8">
                <div className="p-8 bg-white/[0.02] border border-white/5 rounded-[2.5rem] space-y-8 backdrop-blur-xl">
                   <h3 className="text-[11px] font-black uppercase tracking-[0.4em] text-blue-500">System Metrics</h3>
                   <div className="space-y-6">
                      <div className="flex justify-between items-center text-[11px] font-bold uppercase tracking-widest">
                         <span className="text-gray-600 italic">Temporal Origin</span>
                         <span className="text-white">{date}</span>
                      </div>
                      <div className="flex justify-between items-center text-[11px] font-bold uppercase tracking-widest">
                         <span className="text-gray-600 italic">Integrity Rating</span>
                         <span className="text-blue-500">{media.vote_average.toFixed(1)} / 10</span>
                      </div>
                      <div className="flex justify-between items-center text-[11px] font-bold uppercase tracking-widest">
                         <span className="text-gray-600 italic">Data Class</span>
                         <span className="text-white">{type === 'movie' ? 'Cinematic' : 'Series'}</span>
                      </div>
                      <div className="flex justify-between items-center text-[11px] font-bold uppercase tracking-widest">
                         <span className="text-gray-600 italic">Local Link</span>
                   ...
                   <div className="pt-8 border-t border-white/5">
                      <div className="flex items-center gap-3 p-4 bg-blue-500/5 rounded-2xl border border-blue-500/10">
                        ...
                        <span className="text-[9px] font-black text-blue-500/80 uppercase tracking-widest">Secure Tunnel Established</span>
                      </div>
                   </div>
                </div>
             </div>
